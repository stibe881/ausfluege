...
const AddExcursion = () => {
  ...
  const initializeGoogleMaps = async () => {
    console.warn('Google Maps integration disabled - API key needed');
    // For now, disable Google Maps until proper API key is provided
    // This allows the rest of the form to work normally
  };
  ...
  const loadOptions = async () => {
    try {
      ...
      // Load default regions for Switzerland
      if (formData.country) {
        loadRegionsForCountry(formData.country);
      }
    } catch (error) {
